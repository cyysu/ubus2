...
static const char *format_type(void *priv, struct blob_attr *attr)
{
	static const char * const attr_types[] = {
		[BLOB_ATTR_INT8] = "\"Boolean\"",
		[BLOB_ATTR_INT32] = "\"Integer\"",
		[BLOB_ATTR_STRING] = "\"String\"",
		[BLOB_ATTR_ARRAY] = "\"Array\"",
		[BLOB_ATTR_TABLE] = "\"Table\"",
	};
	const char *type = NULL;
	int typeid;

	if (blob_attr_type(attr) != BLOB_ATTR_INT32)
		return NULL;

	typeid = blob_attr_get_u32(attr);
...
static void receive_call_result_data(struct ubus_request *req, int type, struct blob_attr *msg)
{
	...
	str = blob_buf_format_json_indent(msg, true, simple_output ? -1 : 0);
	printf("%s\n", str);
	free(str);
}
			  ...
			  const char *type, struct blob_attr *msg)
{
	...
	str = blob_buf_format_json(msg, true);
	printf("{ \"%s\": %s }\n", type, str);
	fflush(stdout);
	free(str);
}
...
static int ubus_cli_send(struct ubus_context *ctx, int argc, char **argv)
{
	...
	return ubus_send_event(ctx, argv[0], blob_buf_head(&buf));
}
...
static void wait_event_cb(struct ubus_context *ctx, struct ubus_event_handler *ev,
			  const char *type, struct blob_attr *msg)
{
	static const struct blob_attr_policy policy = {
		"path", BLOB_ATTR_STRING
	};
	...
	blob_attr_parse(msg, &attr, &policy, 1);
	if (!attr)
		return;

	path = blob_attr_data(attr);
